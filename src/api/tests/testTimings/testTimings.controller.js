...
export async function getTestTimings(req, res) {
    try {
        ...
        let [testIdConfirmation,branchConfirmation]= await Promise.all([TestTimings.findOne({testId}),InstituteHierarchy.findOne({childCode:branch})]);
        ...
        if(branch) matchQuery.hierarchyId=branch
        //query test data !!!
        ...
        const resData = await TestTimings.aggregate([{$match:matchQuery},
            {$lookup: {from:'institutehierarchies', localField: 'hierarchyId', 
            foreignField:'childCode', as: "branch"}},{$skip:skip},{$limit:limit},
            {$project:{_id:1,testId:1,class:1,orientations:1,startTime:1,endTime:1,
            duration:1,'branch.childCode':1,'branch.child':1}}]);
        if (!resData) {
